...
До появления нативной поддержки JSON в [[Entity Framework|EF Core]] 7/8, стандартным паттерном для хранения списков или сложных объектов внутри одной ячейки была конвертация в JSON-строку.
...
Но для ссылочных типов, проходящих через конвертер (как `List<string>` в примере выше), [[Entity Framework|EF Core]] по умолчанию сравнивает **ссылки**.
...
2.  **Проблема с [[LINQ]]:** Если вы напишете `.Where(u => u.CustomerId == new CustomerId(5))`, [[Entity Framework|EF Core]] сможет это транслировать (преобразует 5 в int и отправит запрос).
