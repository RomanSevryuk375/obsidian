...
Эти атрибуты влияют и на структуру БД (создают ограничения), и на валидацию в [[ASP.NET]][[Web API]] (возвращают 400 Bad Request, если данные неверны).
...
*   **`[NotMapped]`**
    Говорит EF Core **игнорировать** это свойство. Оно не создаст колонку в базе данных.
...
| **Валидация:** Работает и для БД, и для UI (API).                                                               | **Чистота:** Модели остаются чистыми ([[POCO (Plain Old CLR Object)]]), без зависимостей от EF. |
| **Ограничения:** Нельзя настроить составные ключи (до EF Core 5+) или сложные связи Many-to-Many с доп. полями. | **Гибкость:** Позволяет переопределять настройки для разных баз данных.  |
